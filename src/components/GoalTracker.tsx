...
import { Target, Plus, X, RotateCcw, ChevronUp, Minus, Award } from 'lucide-react';
...
const GoalTracker: React.FC = () => {
  ...
  const decrement = (id: string) => {
    setGoals(prev => prev.map(g => g.id === id ? { ...g, current: Math.max(g.current - 1, 0) } : g));
  };
            ...
            <div className="glass rounded-xl p-3 space-y-2.5">
              ...
              <div className="flex gap-2 flex-wrap">
                <input
                  ...
                  placeholder="Target"
                  className="w-20 glass rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-transparent outline-none tabular-nums"
                />
                <input
                  value={unit}
                  onChange={e => setUnit(e.target.value)}
                  placeholder="unit"
                  className="w-20 glass rounded-lg px-2.5 py-1.5 text-xs text-foreground bg-transparent outline-none"
                />
                  ...
                  {(['daily', 'weekly'] as const).map(p => (
                    <motion.button
                      ...
                      className={`text-[10px] px-2.5 py-1 rounded-lg font-medium transition-all ${
                        period === p ? 'bg-primary/20 text-primary' : 'text-muted-foreground/50 hover:text-muted-foreground'
                      }`}
                    >
          ...
          {goals.map(goal => {
            ...
            return (
              <motion.div
                key={goal.id}
                layout
                className={`glass rounded-xl p-3.5 group transition-all ${
                ...
                <div className="flex items-start justify-between mb-2.5">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[9px] px-1.5 py-px rounded-md bg-white/[0.04] text-muted-foreground/60 font-medium">
                        {goal.period}
                      </span>
                    </div>
                  </div>
                  <span className={`text-xs font-bold tabular-nums ${isComplete ? 'text-emerald-400' : 'text-foreground'}`}>
                    {pct}%
                  </span>
                </div>
                  ...
                  {!isComplete ? (
                    <div className="flex gap-1">
                      <motion.button
                        ...
                        className="text-[10px] w-6 h-6 rounded-md bg-white/[0.04] text-muted-foreground hover:text-foreground flex items-center justify-center"
                      >
                      ...
                      <motion.button
                        ...
                        className="text-[10px] px-3 h-6 rounded-md bg-primary/15 text-primary font-medium flex items-center justify-center gap-0.5"
                      >
                      ...
                      <motion.button
                        ...
                        className="text-[10px] px-3 h-6 rounded-md bg-primary/10 text-primary/70 font-medium flex items-center justify-center gap-0.5"
                      >
