...
const StatsPage = () => {
  ...
  return (
    <PageLayout>
      <motion.h1 variants={staggerItem} initial="hidden" animate="show" className="text-2xl font-bold text-foreground mb-6">Pomodoro Stats</motion.h1>

      <motion.div variants={staggerContainer} initial="hidden" animate="show" className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {statCards.map((s) => (
            ...
            {hourDist.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={hourDist}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} />
                  <Bar dataKey="count" fill="hsl(200 70% 55%)" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
