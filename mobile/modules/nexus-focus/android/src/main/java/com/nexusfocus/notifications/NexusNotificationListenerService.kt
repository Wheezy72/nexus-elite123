...
class NexusNotificationListenerService : NotificationListenerService() {
  ...
  private fun shouldBlock(sbn: StatusBarNotification, packageName: String, category: String): Boolean {
    ...
    if (level == com.nexusfocus.notifications.models.BlockLevel.IMPORTANT) {
    ...
    if (
      level == com.nexusfocus.notifications.models.BlockLevel.FULL ||
      level == com.nexusfocus.notifications.models.BlockLevel.SILENT ||
      level == com.nexusfocus.notifications.models.BlockLevel.VIBRATE
    ) {
      return true
    }
  ...
  private fun maybeVibrate() {
    val level = FocusController.getBlockLevel(this)
    if (level != com.nexusfocus.notifications.models.BlockLevel.VIBRATE) return
